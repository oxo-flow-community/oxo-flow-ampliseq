#!/usr/bin/env sh
# Verbatim port of nf-core/ampliseq's prefilter_sidle_tablerecon.sh
# (bin/prefilter_sidle_tablerecon.sh) used by SIDLE_TABLERECON.
#
# Usage: sidle_prefilter_tablerecon.sh <min_counts> <regional_table.qza...>

set -eu

if [ "$#" -lt 2 ]; then
    echo "Usage: sidle_prefilter_tablerecon.sh <min_counts> <regional_table.qza...>" >&2
    exit 1
fi
